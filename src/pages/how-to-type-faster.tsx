    ...
    <>
      ...
      >
        <DocumentSection title="1. Fix accuracy before chasing speed">
          ...
          <p>If your accuracy is below ~95%, that is your bottleneck, full stop. Slow down until you are hitting 97–98%, then let speed climb from there. It feels counterintuitive and it is the fastest route up.</p>
        ...
        <DocumentSection title="3. Drill the weakness, then re-measure">
          <p>Once you know the weak keys, drill only those — short, dense reps on the exact characters and transitions that are slow, instead of whole paragraphs. A minute of targeted drilling moves the needle more than ten minutes of general typing.</p>
          <p>The step people skip is the second half: <strong>re-measure</strong>. Run the same test again and look at the before→after delta. If the number moved, the drill worked; if it did not, drill something else. TypeCafe wires this loop together — each diagnosis opens a one-click drill built from those keys, and dropping you back into a re-measure.</p>
        ...
        <DocumentSection title="4. Make it a short daily habit">
          <p>Typing speed is motor memory, and motor memory is built by frequency, not marathon sessions. Five focused minutes a day beats an hour once a week. Consistency is the whole game.</p>
          <p>The daily challenge gives everyone the same 30-second text each day, so you have one small, repeatable rep to show up for — and a streak that makes showing up sticky.</p>
        ...
        <DocumentSection title="6. Technique that actually pays off">
          <ul>
            <li><strong>Touch type.</strong> Keep your fingers on the home row (<code>asdf</code> / <code>jkl;</code>) and let each finger own its columns. If you are hunting and pecking, learning this is the single biggest speed unlock there is.</li>

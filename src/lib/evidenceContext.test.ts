...
describe("evidence context", () => {
    ...
    it("parses versioned Guided and Custom Practice records conservatively", () => {
        ...
        expect(parsePracticeRecord({ ...guided, durationSeconds: 45 })).toMatchObject({ durationSeconds: 45 })
        expect(parsePracticeRecord({ ...guided, durationSeconds: 0 })).toBeNull()
        expect(parsePracticeRecord({ ...guided, durationSeconds: 3_601 })).toBeNull()
        expect(parsePracticeRecord({ ...guided, durationSeconds: 1.5 })).toBeNull()

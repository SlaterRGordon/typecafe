...
describe("Custom Grams preferences", () => {
    it("accepts direct mixed Unicode Grams and repairs invalid storage independently", () => {
        ...
        expect(parseCustomGramsPracticePreferences({ grams: ["TH", "L’esprit", "co‑operate"], durationSeconds: 120, textStyle: "pseudo" }))
            .toEqual({ grams: ["th", "l'esprit", "co-operate"], durationSeconds: 120, infinite: false, textStyle: "pseudo" })
        expect(parseCustomGramsPracticePreferences({ grams: ["TH", "été", "tion"], durationSeconds: 120, textStyle: "pseudo" }))
            .toEqual({ grams: ["th", "été", "tion"], durationSeconds: 120, infinite: false, textStyle: "pseudo" })
        expect(parseCustomGramsPracticePreferences({ grams: ["x"], durationSeconds: 45, textStyle: "dense" }))
            .toEqual({ grams: ["th", "the", "tion"], durationSeconds: 45, infinite: false, textStyle: "varied" })
        expect(parseCustomGramsPracticePreferences({ grams: ["x"], durationSeconds: 0 }))
            .toEqual({ grams: ["th", "the", "tion"], durationSeconds: 60, infinite: false, textStyle: "varied" })
    })
})

...
Measure a Test
  ...
  -> open a Target-bound Guided Drill in Practice
  -> take an ordinary Test
  -> show natural proof and the Delta in Progress
```
...
flowchart TD
    ...
    Score --> Practice
    Practice --> ReMeasure["/ — Ordinary Test"]
    ReMeasure --> Score
...
- Primary rail/bottom navigation: Home, Practice, Progress, Train, Leaderboard,
...
- The shortest improvement path is `Home → Score/diagnosis → Guided Practice
  → Home Test → Progress`; Progress Targets are primary direct entries into the
  same Guided Practice workspace.
...
|---|---|---|
| `/` | ordinary Timed, Words, Relaxed, or Quote Test and result | `pages/index.tsx`, `typer/Typer.tsx`, `typer/Text.tsx`, `config/ModeBar.tsx` |
| `/practice` | recommendation landing, Target-bound Guided Drill, Custom Keys/Grams, and Practice recap | `pages/practice.tsx`, `lib/guidedPractice.ts`, `lib/customKeysPractice.ts`, `lib/customGramsPractice.ts` |
| `/drill` | compatibility redirect for historical Drill links; not a user-facing destination | `pages/drill.tsx`, `lib/drillCompatibility.ts` |
...
ModeBar + persisted settings
  ...
  -> each supported Finding links directly to Guided Practice
  -> Practice records response separately and returns measurement to `/`
```
...
- `src/lib/guidedPractice.ts`: Target-bound Practice setup, compilation, and recap.
...
Three local-first settings affect ordinary Test generation and teaching:

- `useTestSettings()` owns ordinary Test configuration in
  `typecafe:testSettings` and sanitizes retired Home modes on read.
...
`components/GuestImport.tsx` imports all guest Test and Practice timeline evidence on sign-in and clears each

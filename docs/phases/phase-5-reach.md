...
Reach amplifies what exists — that's why it's Phase 5. Marketing channels and tactics live in [growth.md](../growth.md); this doc covers only the *product* work that creates reach.

---

## 5.1 Code mode (L) — one mode, three wins
...
- Appears in the Phase 1 mode bar; ranked boards keep code separate from prose
- `/typing-test-for-programmers` lands directly in code mode with crawlable copy

## 5.2 Quote & passage packs (M)
...
- A "quotes" text source within Normal mode (not a fifth mode — the mode bar stays clean)
...
- "What is a good typing speed" page seeded with published research numbers; **switches to our own anonymized distribution only when volume justifies it (Phase 6 honesty rule — no fake authority)**
- Technical hygiene: per-page meta/canonicals audit, sitemap already exists

## 5.5 Embeddable widget (M)
...
## 5.6 Deliberately deferred

- **Layout-transition coaching** (QWERTY→Colemak) → Phase 6: real niche, but it needs per-layout key mapping through the whole stats pipeline — a structural change that shouldn't ride along casually
- **Hardware/QMK tie-ins** → Phase 6: needs community standing first

...
- 2026-06-15 — *slice 2, score-card transition framing:* the `DiagnosisPanel` now shows this test's slowest transitions via `worstTransitions(aggregateTransitions(events))` — "b→r takes you 2.1× your average pace → [Drill br]" — replacing the old generic "Costliest transitions" line. ponytail: drills the pair into **Practice** (the working handoff), not grams — grams pre-load needs a custom-gram engine path (grams is a corpus-driven leveled progression, not a single-bigram drill) and is deferred. No bespoke e2e (the lib is unit-tested and the render mirrors the e2e-covered `/progress` card); the existing diagnosis test confirms the restructured panel still renders.
- *Next slices:* drill **into grams** pre-loaded with the bigram (needs a grams custom-gram path); grams auto-suggest reordered by the user's weak transitions; sync-on-signup batch-import of the guest mirror (the local data persists meanwhile).

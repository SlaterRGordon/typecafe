...
## The door-keepers (rules for Phases 0–5, in force now)
...
4. **`CoachContext` JSON contract** (Phase 3) stays renderer-agnostic — heuristics, LLM, or future models are swappable voices
...
**Trigger:** Phases 0–3 stable; this becomes the most buildable moonshot — likely first off this list.
...
**Can't yet because:** schema still churns through Phase 3 — exporting now would freeze mistakes. Export-your-data (a JSON dump button) is cheap and could ship early as the privacy-posture proof.
...
**Can't yet because:** the data doesn't exist and *we do not fake scale* — no synthetic findings, no "our data shows" until it does (the Phase 5 SEO rule applies here too).
...
**Prerequisite:** anti-cheat far beyond Phase 4's sanity checks (locked environment, replay analysis, possibly liveness); brand authority that makes the certificate worth citing.
**Free-forever note:** certification ships free if it ships (locked constraint) — its value to us is authority, not revenue.
**Can't yet because:** certification from a site nobody knows certifies nothing. Earliest after Phase 5 traction.
...
**Prerequisite:** per-user transition data (Phase 3 ✓ when done); a generation approach — weighted Markov over the corpus is buildable *without* ML and should be tried first as a Phase 3 extension; actual learned models need the research dataset.
**Path:** Markov version graduates out of this doc into Phase 3 scope as soon as TransitionStat has real data.
...
**Prerequisite:** Phase 4 boards + actual communities asking for it.
